...
module BackfireRailsQueryClassMethods
  ...
  def acts_as_backfire_query(options = {})
    cattr_accessor :query_name_field, :query_expression_field, :query_fact_field
...
module BackfireRailsQueryInstanceMethods
  ...
  def query_instance
    @query_instance= Query.new(self.query_name, Expression.parse(self.query_expression), self.query_fact_name) if @query_instance.nil?
